...
public:
	...
	// 增加一个成员模板
	template <class It>
	DoubleQueue(It begin, It end):_head(NULL),_tail(NULL),_size(0)
	{
		while(begin != end)
		{
			push_back(*begin);
...
int main()
{
	...
	// 测试迭代器构造函数
	std::string greetings[] = {"Hello", "Hi","Morning"};
	DoubleQueue<std::string> q(greetings+1, greetings+3);
	std::cout << "greetings=" << q << std::endl;
}